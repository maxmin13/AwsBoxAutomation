...
export default function AccountPage() {
  ...
  async function handleMfaActivate() {
  ...
  function handleCreateIamUser() {
    ...
    withAuth(async () => {
      ...
      if (res.ok && res.accessKeyId && res.secretAccessKey) {
        ...
        if (res.rootKeysDeleted) setIamRootKeysDeleted(true)
      } else {
  ...
  const smsCard = (
    <div className="bg-zinc-800 border border-zinc-700 rounded-lg p-4 flex flex-col gap-3">
      ...
      <button onClick={handleEnableSmsAlert} {...iamGated(!isValidPhone(smsPhone) || smsBusy || smsDone)}>
  ...
  return (
    <div className="flex flex-col gap-5 max-w-lg">
      ...
      <div className="flex items-start justify-between">
        ...
        <button onClick={() => { setWizardStep(1); setPageMode('wizard') }}
